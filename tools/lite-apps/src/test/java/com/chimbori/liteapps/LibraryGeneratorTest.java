...
public class LibraryGeneratorTest {
  ...
  public void testIndexJsonIsWellFormedAndReformat() throws IOException {
    TestHelpers.assertJsonIsWellFormedAndReformat(FilePaths.SRC_INDEX_JSON);
  }
  ...
  public void testThatEveryManifestIsIncludedInIndexJsonFile() throws IOException {
    ...
    File[] files = FilePaths.SRC_ROOT_DIR.listFiles();
    for (File file : files) {
      if (file.isDirectory()) {
    ...
    JSONArray library = new JSONArray(FileUtils.readFully(new FileInputStream(FilePaths.SRC_INDEX_JSON)));
